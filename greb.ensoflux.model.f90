...
! numerical parameter
  ...
  integer, parameter :: ndt_days  = 24*3600/dt        ! number of timesteps per day (it's 2)
...
! declare climate fields
  real, dimension(xdim,ydim)          ::  z_topo, glacier,z_ocean,sstanom
  real, dimension(xdim,ydim,nstep_yr) ::  Tclim, uclim, vclim, qclim, mldclim, Toclim, cldclim
  real, dimension(xdim,ydim,nstep_yr) ::  TF_anom, TF_correct, qF_correct, ToF_correct, swetclim, dTrad
...
! scenario run
    ...
    ! Add anomalous winds, smc, clouds to climatology, nlt 3/2015
    if (anom == 'wnd') then
        print*,'anom = wind'
        vclim = vclim + vanom               
        where (vclim(:,:,:) >= 0.0) 
    ...
    else if ((anom == 'all').or.(anom == 'ast')) then
        print*,'anom = '
        print *,anom
        print*,'wind anom'
        vclim = vclim + vanom               
...
!             factor = 0.008
!             sinfactor = sin(2*pi*it/(730*4))*factor
!             Ts1=Ts0+sstanom*factor
!         end if
